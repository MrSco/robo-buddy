...
export const PROVIDERS: Record<string, { endpoint: string; model: string; stt: string; keyHint: string }> = {
  groq: {
    ...
    keyHint: "free at console.groq.com/keys",
  },
  gemini: {
...
export class TalkTab implements TabModule {
  ...
  init(ctx: SettingsContext) {
    ...
    this.els.chatEndpoint.addEventListener("change", () => void ctx.commit({ chatEndpoint: this.els.chatEndpoint.value.trim() }));
    this.els.chatModel.addEventListener("change", () => void ctx.commit({ chatModel: this.els.chatModel.value.trim() }));
    this.els.chatStt.addEventListener("change", () => void ctx.commit({ chatSttModel: this.els.chatStt.value.trim() }));
    this.els.chatVoice.addEventListener("change", () => void ctx.commit({ chatVoice: this.els.chatVoice.checked }));
    this.els.chatLines.addEventListener("change", () => void ctx.commit({ chatGenerateLines: this.els.chatLines.checked }));
    this.els.chatCap.addEventListener("change", () =>
      void ctx.commit({ chatDailyCap: Math.max(0, Math.round(Number(this.els.chatCap.value) || 0)) })
    );

    this.els.ttsEngine.addEventListener("change", () => {
      this.els.piperFields.hidden = this.els.ttsEngine.value !== "piper";
      void ctx.commit({ ttsEngine: this.els.ttsEngine.value });
    ...
    this.els.personality.addEventListener("change", async () => {
      ...
      try {
        ...
        await ctx.commit({ personality: selected });
  ...
  hotkeyFrom(e: KeyboardEvent): string | null {
    const parts: string[] = [];
  ...
  wireLive() {
    ...
    this.els.liveVoice.addEventListener("change", () => void this.ctx.commit({ liveVoice: this.els.liveVoice.value.trim() }));
  ...
  wireVoicePreview() {
    ...
    this.els.voicePreview.addEventListener("click", () => {
      const name = this.ctx.getCurrentManifest()?.name ?? "your buddy";
  ...
  async refreshKeyStatus() {
    try {
      ...
      this.els.chatKeyStatus.textContent = (has ? "A key is saved." : "No key saved.") + (used ? ` ${used} requests today.` : "");
    } catch {
  ...
  editorToProfile(id: string): Personality {
  ...
  editorSnapshot() {
    return JSON.stringify(
      Array.from(this.els.pedit.querySelectorAll<HTMLInputElement | HTMLTextAreaElement>("input, textarea")).map((el) => el.value)
    );
  }
