...
export class WindowTab implements TabModule {
  ...
  init(ctx: SettingsContext) {
    ...
    this.els.fullscreen.addEventListener("change", () => ctx.commit({ hideWhenFullscreen: this.els.fullscreen.checked }));
    this.els.taskbar.addEventListener("change", () => ctx.commit({ standOnTaskbar: this.els.taskbar.checked }));
